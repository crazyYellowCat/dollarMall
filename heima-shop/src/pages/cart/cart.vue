...
//计算选中商品列表
const selectedInCartList = computed(() => {
  return cartList.value.filter((v) => v.selected)
})
//购物车选中商品总数
const selectedInCartListCount = computed(() => {
  return selectedInCartList.value.reduce((sum, item) => sum + item.count, 0)
})
//计算选中总金额
const selectedInCartListMoney = computed(() => {
  return selectedInCartList.value.reduce((sum, item) => sum + item.count * item.nowPrice, 0
  ).toFixed(2)
})
// 结算按钮
const gotoPayment = () => {
  if (selectedInCartListCount.value === 0) {
  ...
  <scroll-view scroll-y class="scroll-view">
    ...
    <template v-if="memberStore.profile">
      ...
      <view class="cart-list" v-if="true">
        ...
        <uni-swipe-action>
          <!-- 滑动操作项 -->
          <uni-swipe-action-item v-for="(item, index) in cartList" :key="item.id" class="cart-swipe">
            <!-- 商品信息 -->
            <view class="goods">
              <!-- 选中状态 -->
      ...
      <view class="toolbar">
        ...
        <text class="amount">{{ selectedInCartListMoney }}</text>
        <view class="button-grounp">
          <view class="button payment-button" :class="{ disabled: true }" @tap="gotoPayment">
            去结算({{ selectedInCartListCount }}) </view>
        </view>
      </view>
    </template>
    <!-- 未登录: 提示登录 -->
