...
import { getHotRecommondAPI } from '@/services/hot'
...
let current = hotMap.find((item) => query.type === item.type)
uni.setNavigationBarTitle({ title: current!.title })
console.log(query)
let banner = ref('')
//tab
let subTypes = ref<(SubTypeItem & { finish?: boolean })[]>([])
//tab高亮
let activeIndex = ref(0)
const getHotRecommondData = async () => {
    const res = await getHotRecommondAPI(current!.url, {
...
const onScrolltolower = async () => {
    ...
    //currentType.goodsItems.items.push(...newsubTypes.goodsItems.items)
    subTypes.value[activeIndex.value].goodsItems.items.push(...newsubTypes.goodsItems.items)

}
onLoad(async () => {
    await getHotRecommondData()
})
</script>
    ...
    <view class="viewport">
        ...
        <view class="tabs">
            <text class="text" v-for="(item, index) in  subTypes  " :key='item.id'
                :class="{ active: index === activeIndex }" @tap="activeIndex = index">{{ item.title
                }} </text>

        </view>
        <!-- 推荐列表 -->
        <scroll-view scroll-y class="scroll-view" v-for="(item, index) in subTypes" v-show="index === activeIndex"
