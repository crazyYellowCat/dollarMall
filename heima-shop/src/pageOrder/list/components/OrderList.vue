...
<template>
    <scroll-view scroll-y class="orders" refresher-enabled enable-back-to-top :refresher-triggered="isTriggered"
        ...
        <view class="card" v-for="order in orderList" :key="order.id">
            ...
            <navigator v-for="item in order.skus" :key="item.id" class="goods"
                :url="`/pageOrder/detail/detail?id=${order.id}`" hover-class="none">
                <view class="cover">
            ...
            <view class="action">
                ...
                <template v-else>
                    <navigator class="button secondary" :url="`/pageOrder/create/create?orderId=id`" hover-class="none">
                        再次购买
                    </navigator>
                    <!-- 待收货状态: 展示确认收货 -->
                    <view v-if="order.orderState === OrderState.DaiShouHuo" class="button primary"
                        @tap="onOrderConfirm(order.id)">确认收货</view>
                </template>
            </view>
        </view>
        <!-- 底部提示文字 -->
