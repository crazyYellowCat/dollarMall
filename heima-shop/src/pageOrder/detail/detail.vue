    ...
    <scroll-view scroll-y class="viewport" id="scroller" @scrolltolower="onScrolltolower" enable-back-to-top>
        <template v-if="order?.orderState === OrderState.DaiFuKuan">
            ...
            <view class="overview" :style="{ paddingTop: safeAreaInsets!.top + 20 + 'px' }">
                ...
                <template v-else>
                    ...
                    <view class="button-group">
                        <navigator class="button" :url="`/pageOrder/create/create?orderId=${query.id}`" hover-class="none">
                            再次购买
                        </navigator>
            ...
            <view class="shipment">
                ...
                <!-- 用户收货地址 -->
                <view class="locate">
                    <view class="user"> {{ order.receiverContact }}{{ order.receiverAddress }} </view>
                ...
                <view class="item">
                    <navigator class="navigator" v-for="item in order.skus" :key="item.id"
                        ...
                        <view class="meta">
                            ...
                            <view class="quantity">x1{{ item.quantity }}</view>
                        </view>
                    </navigator>
                    <!-- 待评价状态:展示按钮 -->
                    <view class="action" v-if="(order.orderState as Number) === OrderState.DaiPingJia">
            ...
            <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
                ...
                <!-- 其他订单状态:按需展示按钮 -->
                <template v-else>
                    <navigator class="button secondary" :url="`/pageOrder/create/create?orderId=${query.id}`"
                        hover-class="none">
                        再次购买
                    </navigator>
                    <!-- 待收货状态: 展示确认收货 -->
                    <view class="button primary" @tap="onOrderConfirm" v-if="order!.orderState === OrderState.DaiShouHuo">
                        确认收货 </view>
                    <!-- 待评价状态: 展示去评价 -->
