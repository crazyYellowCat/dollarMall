...
<template>
    <scroll-view scroll-y class="viewport">
        ...
        <!-- 商品信息 -->
        <view class="goods">
            <navigator v-for="good in orderPre?.goods" :key="good.id" :url="`/pages/goods/goods?${good.id}`" class="item"
                hover-class="none">
                <image class="picture" :src="good.picture" />
                <view class="meta">
                    <view class="name ellipsis"> {{ good.name }} </view>
                    <view class="attrs">{{ good.attrsText }}</view>
                    <view class="prices">
    ...
    <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        ...
        <view class="button" :class="{ disabled: true }" @tap="onOrderSubmit"> 提交订单 </view>
    </view>
</template>
